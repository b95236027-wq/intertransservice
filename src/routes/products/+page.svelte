<script lang="ts">
	import { getProductsByCapacityPairs, getProductCapacity, getProductMaxHeight } from '$lib/utils/products';
	import * as m from '$lib/paraglide/messages';

	const productPairs = getProductsByCapacityPairs();
</script>
...
<section class="py-20 bg-gray-100">
	<div class="container mx-auto px-4">
		<div class="space-y-12">
			{#each productPairs as pair}
				<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
					<!-- SANY Product (Left) -->
					{#if pair.sany}
						<a href="/products/{pair.sany.slug}" class="group">
							<div class="card-industrial bg-white h-full overflow-hidden">
								...
								<div class="relative h-64 overflow-hidden bg-gray-200">
									...
									<div class="absolute bottom-4 left-4 right-4">
										<div class="text-[#c9a227] text-sm font-semibold mb-1">{pair.sany.type}</div>
					...
					{:else}
						<div class="opacity-50 flex items-center justify-center h-64 bg-gray-200 rounded-lg">
							<p class="text-gray-500">SANY {pair.capacity} - Coming Soon</p>
						</div>
					{/if}
					...
					{:else}
						<div class="opacity-50 flex items-center justify-center h-64 bg-gray-200 rounded-lg">
							<p class="text-gray-500">XCMG {pair.capacity} - Coming Soon</p>
						</div>
					{/if}
				</div>
			{/each}
